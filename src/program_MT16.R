program <-
  function(D_matrix, k=NULL, cancer_type) {
    ##
    ...
    input = "/Users/ashwin/Documents/Projects/COMETH/src/compExplore/DeconCellTypesAPP/src"
    ...
    if ( !is.null(x = D_matrix) ) {
      ...
      #keep = intersect(rownames(mt),rownames(D_matrix))
      keep = intersect(rownames(mt),rownames(T))
      mt = mt[keep,]
      T = T[keep,]
      print(paste0("dim of D_matrix is :", dim(T)))
      print(paste0(length(keep), " genes are kept as reference markers"))
      
      ML = CellMix::MarkerList()
      ML@.Data <- tapply(as.character(mt$gene),as.character(mt$CT),list)
      
      RESULTS <- CellMix::ged(as.matrix(T), ML, method = "ssKL", sscale = FALSE, maxIter=500, log = FALSE)
