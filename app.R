...
ui <- fluidPage(
  ...
  navbarPage("DeconExplorer",
             ...
             tabPanel(
               ...
               p("Cattell’s rule is here suggested for choosing K [1]: it states that components corresponding to eigenvalues to the left of the straight line should be retained. When the actual number of different cell types is equal to K, we expect that there are (K-1) eigenvalues would correspond to the mixture of cell types and that other eigenvalues would correspond to noise (or other unaccounted for confounders). Indeed, one PCA axis is needed to separate two types, two axes for three types, etc. However, when not accounting for confounders, Cattell’s rule overestimates the number of PCs."),
               
               h1("Enrichment analysis"),
               p("This module helps in the biological interpretation of the components identified by unsupervised approaches. It proposes to perform Gene Set Enrichment analysis (fgsea R package [2]) on the cell-type gene marker database 'CellMatch' [3]. Do note that ICA-based methods are subjected to a reorientation of the components, as proposed in the deconica R package [4]."),
               
               h1("References"),
               tags$a(href = "https://cancer-heterogeneity.github.io/cometh.html", "This app was developed within the COMETH program"), tags$br(),tags$br(),
               
               p("[1] Cattell RB. The scree test for the number of factors. Multivariate Behav Res.2010;1:245–76.", tags$br(),
                 "[2] Korotkevich G, Sukhov V, Sergushichev A (2019). “Fast gene set enrichment analysis.” bioRxiv. doi: 10.1101/060012.", tags$br(),
                 "[3] Shao et al., scCATCH:Automatic Annotation on Cell Types of Clusters from Single-Cell RNA Sequencing Data, iScience, Volume 23, Issue 3, 27 March 2020. doi: 10.1016/j.isci.2020.100882" , tags$br(),
